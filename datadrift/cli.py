...
from datadrift.contracts import format_default_contract_block
from datadrift.drift import compute_data_drift
from datadrift.git_tools import get_git_root, get_working_changes
from datadrift.specs import DatadriftSpec, extract_datadrift_spec, parse_datadrift_spec
from datadrift.workgraph import Workgraph, find_workgraph_dir
...
def _maybe_write_log(wg: Workgraph, task_id: str, report: dict) -> None:
    ...
    if not findings:
        msg = "Datadrift: OK (no findings)"
    else:
        kinds = ", ".join(sorted({str(f.get("kind")) for f in findings}))
        msg = f"Datadrift: {score} ({kinds})"
        if recs:
...
def _maybe_create_followups(wg: Workgraph, report: dict) -> None:
    ...
    spec = DatadriftSpec.from_raw(spec_raw)
    ...
    for f in findings:
        ...
        if kind != "schema_not_updated":
            continue

        follow_id = f"drift-data-{task_id}"
        title = f"data: {task_title}"
        touch = list(spec.migrations or []) + list(spec.schema_files or [])
        desc = (
            "Update schema/migrations to match current code changes.\n\n"
            "Context:\n"
            f"- Origin task: {task_id}\n"
            f"- Finding: {f.get('summary')}\n\n"
            + format_default_contract_block(mode="core", objective=title, touch=touch)
            + "\n"
            + (report.get("_datadrift_block") or "").strip()
            + "\n"
        )
        wg.ensure_task(
            task_id=follow_id,
...
def cmd_wg_check(args: argparse.Namespace) -> int:
    ...
    raw_block = extract_datadrift_spec(description)
    if raw_block is None:
        report = {
            ...
            "score": "green",
            "spec": None,
            "telemetry": {"note": "no datadrift block"},
            "findings": [],
            "recommendations": [],
        }
        if args.json:
    ...
    try:
        spec_raw = parse_datadrift_spec(raw_block)
        spec = DatadriftSpec.from_raw(spec_raw)
        ...
        report = {
            ...
            "findings": [
                {
                    "kind": "invalid_datadrift_spec",
                    "severity": "warn",
                    "summary": "datadrift block present but could not be parsed",
                }
            ],
            "recommendations": [
                {
                    "priority": "high",
                    "action": "Fix the datadrift TOML block so it parses",
                    "rationale": "Datadrift can only advise on schema drift when it can read the configuration.",
                }
            ],
        }
        report["_datadrift_block"] = f"```datadrift\n{raw_block}\n```"
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="datadrift")
    ...
    check = wg_sub.add_parser("check", help="Check for schema/migration drift (requires a datadrift block in the task)")
