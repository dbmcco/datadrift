...
# Optional: ignore globs (same ** semantics as specdrift/coredrift).
ignore = [
  "**/*.md",
]
```
...
`datadrift` runs automatically when a task description contains a fenced `datadrift` TOML block. It is also triggered by `driftdriver` during factory cycles and attractor loop passes.

### Per-Task Workflow

1. Add a `datadrift` fence to task descriptions that touch database migrations or schema files
...
- Do not modify `datadrift` spec blocks without understanding the lane contract
